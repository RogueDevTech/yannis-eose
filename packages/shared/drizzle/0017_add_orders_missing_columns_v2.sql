-- Migration: add 3 missing orders columns
-- These columns exist in the Drizzle schema (orders.ts) but were never migrated:
--   delivery_state, customer_gender, preferred_delivery_date
-- Also syncs orders_history and updates the explicit-column INSERT trigger.

-- ── orders ────────────────────────────────────────────────────
ALTER TABLE orders
  ADD COLUMN IF NOT EXISTS delivery_state           text,
  ADD COLUMN IF NOT EXISTS customer_gender           text,
  ADD COLUMN IF NOT EXISTS preferred_delivery_date   text;
