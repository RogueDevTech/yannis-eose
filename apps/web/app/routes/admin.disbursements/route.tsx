...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [fundingRes, usersRes] = await Promise.all([
    apiRequest<unknown>(
      `/trpc/marketing.listFunding?input=${encodeURIComponent(JSON.stringify({ page: 1, limit: 100 }))}`,
      { method: 'GET', cookie },
    ),
    apiRequest<unknown>(
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === 'createFunding') {
    ...
    if (!res.ok) {
      ...
      return json({ error: errorData?.error?.message ?? 'Failed to create disbursement' }, { status: res.status });
    }
