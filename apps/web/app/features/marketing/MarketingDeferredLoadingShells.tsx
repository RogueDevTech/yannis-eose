...
/** Same base labels as `MarketingOrdersPage` before status counts stream (no API). */
const MARKETING_ORDERS_STATUS_OPTIONS_BASE = STATUS_OPTIONS.map((status) => ({
  value: status,
  label: status === 'ALL' ? 'All Statuses' : formatStatus(status),
}));

const MARKETING_ORDERS_SHELL_TABLE_ROWS = 10;

function marketingOrdersShellPlaceholderRows(): Order[] {
  return Array.from({ length: MARKETING_ORDERS_SHELL_TABLE_ROWS }, (_, i) => ({
    id: `__marketing_orders_shell_${i}`,
    customerName: '',
...
const MARKETING_ORDERS_SHELL_TABLE_ROW_DATA = marketingOrdersShellPlaceholderRows();

function marketingOrdersLoadingShellTableColumns(
...
/** Marketing orders — PageHeader + live + filters; stats + table pulse. */
export function MarketingOrdersLoadingShell({
  ...
  isMediaBuyer: boolean;
  liveEvents?: string[];
  /** Match deferred page: extra buyer filter skeleton on HoM / admin */
  showMediaBuyerColumn?: boolean;
}) {
  const [searchParams, setSearchParams] = useSearchParams();
  const [searchQuery, setSearchQuery] = useState(() => searchParams.get('search') ?? '');
  ...
  const selectedStatus = searchParams.get('status') || 'ALL';
  const mediaBuyerIdParam = searchParams.get('mediaBuyerId') || 'ALL';
  const productIdParam = searchParams.get('productId') || 'ALL';
  const campaignIdParam = searchParams.get('campaignId') || 'ALL';

  const mediaBuyerOptions = useMemo(() => {
    const base: { value: string; label: string }[] = [{ value: 'ALL', label: 'All media buyers' }];
    if (showMediaBuyerColumn && mediaBuyerIdParam !== 'ALL') {
      base.push({ value: mediaBuyerIdParam, label: 'Selected media buyer' });
    }
    return base;
  }, [showMediaBuyerColumn, mediaBuyerIdParam]);
  ...
  const campaignOptions = useMemo(() => {
    const base = [{ value: 'ALL', label: 'All forms' }];
    if (campaignIdParam !== 'ALL') {
      base.push({ value: campaignIdParam, label: 'Selected form' });
    }
    return base;
  }, [campaignIdParam]);
  ...
  const badgeCount = useMemo(() => {
    let n = 0;
    if (selectedStatus !== 'ALL') n += 1;
    if (showMediaBuyerColumn && mediaBuyerIdParam !== 'ALL') n += 1;
    if (productIdParam !== 'ALL') n += 1;
    if (campaignIdParam !== 'ALL') n += 1;
    return n;
  }, [selectedStatus, showMediaBuyerColumn, mediaBuyerIdParam, productIdParam, campaignIdParam]);
  ...
  const buyerSelectDisabled = Boolean(showMediaBuyerColumn && mediaBuyerOptions.length <= 1);
  const productSelectDisabled = productOptions.length <= 1;
  const campaignSelectDisabled = campaignOptions.length <= 1;

  const shellTableColumns = useMemo(
    () => marketingOrdersLoadingShellTableColumns(showMediaBuyerColumn),
  ...
  return (
    <div className="space-y-4" aria-busy="true" aria-live="polite">
      <PageHeader
        ...
        description={
          isMediaBuyer ? 'Track your campaign orders and conversion funnel' : 'View orders across all media buyers'
        }
        actions={
          <PageHeaderMobileTools
      ...
      <ToolbarFiltersCollapsible
        ...
        desktopInlineFilters={
          <>
            ...
            {showMediaBuyerColumn ? (
              <SearchableSelect
                id="marketing-orders-shell-buyer"
                value={mediaBuyerIdParam}
                onChange={setMediaBuyer}
                options={mediaBuyerOptions}
                disabled={buyerSelectDisabled}
            ...
            <SearchableSelect
              id="marketing-orders-shell-product"
            ...
            />
            <SearchableSelect
              id="marketing-orders-shell-form"
              value={campaignIdParam}
              onChange={setCampaign}
              options={campaignOptions}
              disabled={campaignSelectDisabled}
              wrapperClassName="w-full min-w-0 sm:w-48"
          ...
          <>
            ...
            {showMediaBuyerColumn ? (
              <div className="space-y-1.5">
                <span className="text-xs font-medium text-app-fg-muted">Media buyer</span>
                <SearchableSelect
                  id="marketing-orders-shell-buyer-sheet"
                  value={mediaBuyerIdParam}
                  onChange={setMediaBuyer}
                  options={mediaBuyerOptions}
                  disabled={buyerSelectDisabled}
            ...
            <div className="space-y-1.5">
              <span className="text-xs font-medium text-app-fg-muted">Product</span>
              <SearchableSelect
                id="marketing-orders-shell-product-sheet"
                value={productIdParam}
                onChange={setProduct}
                options={productOptions}
                disabled={productSelectDisabled}
            ...
            <div className="space-y-1.5">
              ...
              <SearchableSelect
                id="marketing-orders-shell-form-sheet"
                value={campaignIdParam}
                onChange={setCampaign}
                options={campaignOptions}
                disabled={campaignSelectDisabled}
      ...
      <div className="card scroll-mt-4 overflow-hidden p-0">
        ...
        <CompactTable<Order>
          withCard={false}
          columns={shellTableColumns}
          rows={MARKETING_ORDERS_SHELL_TABLE_ROW_DATA}
          rowKey={(row) => row.id}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        <p className="text-sm m-0 min-h-[1.25rem] flex items-center">
          <span className="inline-block h-4 w-52 max-w-[90vw] rounded-md bg-app-border/75 dark:bg-app-border/60 animate-pulse sm:w-72" aria-hidden />
        </p>
