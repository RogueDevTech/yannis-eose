-- Migration 0311: Exclude staff from attendance.
--
-- Adds users.attendance_excluded — when true, the staff member is dropped from
-- the attendance grid, counts, report and the auto-absent cron entirely (they
-- are not tracked). Distinct from attendance_affects_pay (which only governs
-- pay eligibility for tracked staff).
--
-- users history is captured by EXPLICIT-COLUMN trigger functions
-- (yannis_capture_history_insert_users / yannis_capture_history_users). Adding a
-- column to users + users_history is NOT enough — the trigger functions list
-- every column by name, so a new column is silently dropped from history unless
-- the functions are rebuilt. This migration rebuilds BOTH with the COMPLETE
-- current column list (as of mig 0309, which added attendance_affects_pay) plus
-- attendance_excluded appended after it.

-- 1. New column on users
ALTER TABLE users ADD COLUMN IF NOT EXISTS attendance_excluded boolean NOT NULL DEFAULT false;
...
DO $$ BEGIN
  IF EXISTS (
    ...
    ALTER TABLE users_history ADD COLUMN IF NOT EXISTS attendance_excluded boolean;
  END IF;
END $$;

-- 3. Rebuild history trigger functions with the COMPLETE current column list
--    (0309 order) + attendance_excluded appended after attendance_affects_pay.
...
BEGIN
  INSERT INTO users_history (
    ...
    exit_date, exit_reason, attendance_affects_pay, attendance_excluded,
    ...
    NEW.exit_date, NEW.exit_reason, NEW.attendance_affects_pay, NEW.attendance_excluded,
    NEW.valid_from, NEW.valid_to, NEW.modified_by, NEW.created_at, NEW.updated_at;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  INSERT INTO users_history (
    ...
    OLD.exit_date, OLD.exit_reason, OLD.attendance_affects_pay, OLD.attendance_excluded,
    OLD.valid_from, OLD.valid_to, OLD.modified_by, OLD.created_at, OLD.updated_at;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
