...
export const PERMISSIONS: PermissionCatalogEntry[] = [
  ...
  { code: 'orders.followUpConfig', resource: 'orders', action: 'followUpConfig', description: 'Configure follow-up auto-pull rules and trigger manual sync' },
  ...
  { code: 'payroll.run.approve', resource: 'payroll', action: 'run.approve', description: 'Approve payroll pay runs' },
  { code: 'payroll.run.disburse', resource: 'payroll', action: 'run.disburse', description: 'Mark payroll pay runs as disbursed' },
  ...
  { code: 'accounting.read', resource: 'accounting', action: 'read', description: 'View chart of accounts, journal entries, and trial balance (double-entry ledger)' },
  { code: 'accounting.write', resource: 'accounting', action: 'write', description: 'Create accounts, post/reverse journal entries, manage fiscal years' },
  { code: 'accounting.export', resource: 'accounting', action: 'export', description: 'Export trial balance / general ledger as CSV or XLSX' },
...
export const ROLE_PERMISSIONS: Record<string, string[]> = {
  ...
  // ACCOUNTANT = bookkeeping role. Full accounting.* (chart of accounts,
  // journals, GL, reports) + read context for reconciliation. Deliberately
  // NO money movement: no finance.approve / finance.disburse /
  // finance.cashRemittance.* (those belong to FINANCE_OFFICER's operational hat).
  ACCOUNTANT: [
    'accounting.read',
    'accounting.write',
    'accounting.export',
