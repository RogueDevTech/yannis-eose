...
**Transfer Approval Gate (Phase 23, CEO directive 2026-05 — migration 0131):**
Stock leaving a source location now requires explicit sign-off from the source-location authority before any inventory side effect runs. Initiating a transfer no longer always deducts source stock — it does so only when the initiator is themselves the source authority. Otherwise the row starts as `PENDING` and waits for approval.

**State machine:**

| From | To | Trigger | Side effect |
|---|---|---|---|
| — | `IN_TRANSIT` | `inventory.transfer` (initiator IS source authority) | Source stock deducted, `TRANSFER_OUT` movement written, low-stock check scheduled |
| — | `PENDING` | `inventory.transfer` (initiator is NOT source authority) | **No inventory side effect.** Source authority pool notified (`inventory:transfer_pending_approval`) |
| `PENDING` | `IN_TRANSIT` | `inventory.approveTransfer` | Source stock deducted **now**, `TRANSFER_OUT` movement written. Initiator notified (`inventory:transfer_approved`). |
| `PENDING` | `REJECTED` | `inventory.rejectTransfer` (reason ≥ 10 chars) | **Inventory-neutral** — nothing was deducted, nothing to reverse. Initiator notified (`inventory:transfer_rejected`) with the reason. |
| `PENDING` | `CANCELLED` | `inventory.cancelTransfer` (reason ≥ 10 chars) | **Inventory-neutral** — pure status flip. Initiator-only escape hatch. |
| `IN_TRANSIT` | `RECEIVED` / `DISPUTED` / `CANCELLED` | existing verify / cancel flows — UNCHANGED | (cancel from `IN_TRANSIT` still does the dual-leg inventory reversal) |

**Source-authority rule (server-canonical, role-based):** `InventoryService.canApproveSourceTransfer(actor, providerKind)` resolves against the source location's `logistics_providers.kind`:

| Source provider kind | Actors who skip approval (source authority) |
|---|---|
| `WAREHOUSE` (company-owned) | `STOCK_MANAGER`, `BRANCH_ADMIN`, `SUPER_ADMIN`, `ADMIN` |
| `THIRD_PARTY` (external 3PL) | `TPL_MANAGER`, `HEAD_OF_LOGISTICS`, `SUPER_ADMIN`, `ADMIN` |

**`HEAD_OF_LOGISTICS` does NOT skip approval for `WAREHOUSE` sources** — that's the whole point of the gate. Stock managers must consciously sign off on stock leaving the warehouse. Unknown / null provider kind fails closed (requires approval).

**Permission:** `inventory.approveTransfer` (granted to `STOCK_MANAGER`, `TPL_MANAGER`, `HEAD_OF_LOGISTICS`, `BRANCH_ADMIN`; admin-class inherits via `ALL_PERMISSION_CODES`). Approve and reject share the same permission code — they're two sides of the same gate. Server re-checks `canApproveSourceTransfer` on every approve/reject submit; client-side `transfer.canApprove` (computed per-row, per-viewer in `listTransfers`) is just a UI hint to render the buttons.

**Notifications:** `inventory:transfer_pending_approval` fans out via `enqueueCreateForRole` to the source-authority pool (`STOCK_MANAGER` for WAREHOUSE; `TPL_MANAGER` + `HEAD_OF_LOGISTICS` for THIRD_PARTY). `inventory:transfer_approved` and `inventory:transfer_rejected` are single-recipient pings to the original initiator. All three deep-link to `/admin/transfers?status=PENDING&transferId=<id>`.

**UI:** `/admin/transfers` and `/admin/logistics/transfers` (same `TransfersPage`) gain a **Pending approval** tab with a count badge. PENDING rows in the source-authority's view render Approve (one-click) and Reject (modal with reason ≥ 10 chars) buttons in the action column. Approve / Reject use `useOptimisticListPatches` to flip the row's status the instant the action returns success — full Modal+Optimistic UI pattern from CLAUDE.md. Verify (physical receipt at destination) is unchanged — the gate is about *authority to release*, not *physical receipt*.

**Files:** [packages/shared/drizzle/0131_stock_transfers_approval.sql](packages/shared/drizzle/0131_stock_transfers_approval.sql), [apps/api/src/inventory/inventory.service.ts](apps/api/src/inventory/inventory.service.ts) (`canApproveSourceTransfer`, `initiateTransfer`, `approveTransfer`, `rejectTransfer`, `cancelTransfer`, `listTransfers`), [apps/api/src/trpc/routers/inventory.router.ts](apps/api/src/trpc/routers/inventory.router.ts), [packages/shared/src/validators/inventory.ts](packages/shared/src/validators/inventory.ts), [packages/shared/src/notifications/config.ts](packages/shared/src/notifications/config.ts), [apps/web/app/lib/admin-transfers-route.server.ts](apps/web/app/lib/admin-transfers-route.server.ts), [apps/web/app/features/transfers/TransfersPage.tsx](apps/web/app/features/transfers/TransfersPage.tsx).
...
- **Client-side loader cache (LinkedIn-style instant revisit)** — Read-mostly list pages opt in via TWO wired pieces (both required for the true zero-roundtrip effect):
  1. **`<CachedAwait>`** ([cached-await.tsx](apps/web/app/components/ui/cached-await.tsx)) — replaces `<Suspense fallback>` + `<Await resolve>`. Pass `loaderShell={{ <syncShellVar> }}` and `deferredKey="<deferredField>"` props so the cache captures the FULL `useLoaderData()` shape, not just the resolved deferred.
  2. **`export const clientLoader = cachedClientLoader; clientLoader.hydrate = false;`** ([loader-cache.ts](apps/web/app/lib/loader-cache.ts)) — Remix `clientLoader` that runs ON THE CLIENT before each navigation. On cache hit it returns the cached `useLoaderData` shape immediately, **skipping the server roundtrip entirely**. This is what makes revisit feel instant — the route mounts on the same React tick as the click, no fetch, no skeleton, no spinner. `<CachedAwait>` alone (without `clientLoader`) only eliminates the post-mount skeleton flash; the user still feels the ~100–300 ms server loader delay because Remix won't mount the route until the loader returns. On revalidation (fired by `<CachedAwait>` on mount), `clientLoader` falls through to `serverLoader()` so fresh data lands within ~300 ms.
  Storage is a module-level `Map` in [loader-cache.ts](apps/web/app/lib/loader-cache.ts) — NOT React state — keyed by `pathname + search`, 5-minute TTL, LRU 30 entries. Two layers internally: `cache` (deferred resolved value, drives `<CachedAwait>` post-mount paint) + `fullCache` (whole `useLoaderData()` shape, drives `clientLoader`'s zero-roundtrip serve). Cleared on `/auth` mount (logout) so user A's cached data never bleeds into user B's session. **SWR contract**: cache hit renders instantly via `clientLoader`, `<CachedAwait>` fires `useRevalidator().revalidate()` on mount → `clientLoader` runs again, sees a fresh in-flight flag, falls through to `serverLoader()` → fresh data lands within ~300 ms. The in-flight flag has a 5 s TTL so an interrupted navigation doesn't lock the cache out indefinitely. Same-page mutations update the cache via Remix's auto-revalidation; cross-page mutations show stale briefly then refresh. For non-deferred routes (those without a Promise field in the loader return), use [`useCachedLoaderData()`](apps/web/app/hooks/useCachedLoaderData.ts) — wire `clientLoader` the same way. **Opt in** for: list/index pages (orders, ad-spend, funding, leaderboards, payroll, etc.). **Do NOT opt in** for: live socket-driven pages (`/admin/cs/queue`, `/admin/marketing/overview`, `/admin/notifications`), detail pages with on-page mutations (`/admin/orders/$id`, every `*/$id` edit page), forms / wizards (`/admin/marketing/forms/$id/builder`, every `*/new`), real-time PWAs (`/rider/*`), routes with multiple deferred promises (the simple shell+single-deferred shape is required — `admin.marketing.orders._index` intentionally omits `loaderShell`/`deferredKey` because it has both `listPromise` and `secondaryPromise`; it keeps `clientLoader` but the cache always misses). When uncertain, default to NOT caching — a stale flash on a sensitive surface is worse than a fresh-loader-every-click. The currently-cached set lives at the call sites; grep `cachedClientLoader` to enumerate.
...
- Do NOT deduct source stock at `initiateTransfer` when the row starts as `PENDING` — the deduction is the side effect of `approveTransfer`. `rejectTransfer` and `cancelTransfer`-from-PENDING must remain inventory-neutral (no `inventory_levels` writes, no `stock_movements` rows). The whole point of the approval gate is that PENDING is a *promise* of a transfer, not an actual stock move; reversing a non-existent deduction silently corrupts shelf truth. Locked spec: `CLAUDE.md` → "When Building the Third-Party Logistics Module" → "Transfer Approval Gate".
- Do NOT trust the client's `transfer.canApprove` flag to decide who may approve / reject — the server (`InventoryService.approveTransfer` / `rejectTransfer`) re-runs `canApproveSourceTransfer` on every submit. The client flag is purely a UI hint to render the buttons. Inline-checking `actor.role === 'HEAD_OF_LOGISTICS'` etc. anywhere outside `canApproveSourceTransfer` is how the source-authority rule drifts; route all decisions through that helper.
- Do NOT grant `HEAD_OF_LOGISTICS` source-authority for `WAREHOUSE` transfers in `canApproveSourceTransfer`. They get `inventory.approveTransfer` for THIRD_PARTY ↔ THIRD_PARTY moves only; the gate exists so Stock Manager / Branch Admin must consciously sign off on stock leaving the warehouse. CEO directive 2026-05.
