...
function OrdersListPageImpl({
  ...
  const [breakdownModal, setBreakdownModal] = useState<'total' | null>(null);
    ...
    <div className="space-y-4">
      ...
      {deferredLoading && Object.keys(statusCounts).length === 0 ? (
        ...
        <OverviewStatStrip
          ...
          items={[
            {
              label: <span className="flex items-center">Total<StatInfoIcon onClick={() => setBreakdownModal('total')} /></span>,
      ...
      <Modal open={breakdownModal === 'total'} onClose={() => setBreakdownModal(null)} maxWidth="max-w-sm" contentClassName="p-5">
        ...
        <div className="flex items-center justify-between gap-4 py-1.5">
          ...
          <span className="text-sm tabular-nums text-app-fg font-semibold">{Object.entries(statusCounts).filter(([k]) => k !== 'DELETED' && k !== 'CART').reduce((s, [, n]) => s + (n || 0), 0).toLocaleString()}</span>
        </div>
      </Modal>
