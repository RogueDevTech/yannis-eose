...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === 'createRemittance') {
    ...
    const res = await apiRequest<unknown>('/trpc/logistics.createDeliveryRemittance', {
      method: 'POST',
      cookie,
      body: { orderIds, receiptUrls, notes, markReceivedNow, deliveryFees, commitmentFee, posFee, failedDeliveryCost, discount, waybillCost, skipDuplicateWarning },
    });
    if (!res.ok) {
      return json(
    ...
    // Backend returns duplicateWarnings when it detects same customer+product
    // already remitted this month. Surface to the UI for confirmation.
