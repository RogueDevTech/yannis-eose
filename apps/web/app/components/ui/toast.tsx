...
export function useFetcherToast(
  ...
  }, [fetcherData, toast, options?.successMessage]);
}
...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: (id: string) => void }) {
  ...
  const styles: Record<ToastType, { bg: string; icon: string; iconPath: string }> = {
    success: {
      bg: 'bg-success-50 dark:bg-success-900/30 border-success-200 dark:border-success-700/50',
      icon: 'text-success-600 dark:text-success-400',
      iconPath: 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
    },
    error: {
      bg: 'bg-danger-50 dark:bg-danger-900/30 border-danger-200 dark:border-danger-700/50',
      icon: 'text-danger-600 dark:text-danger-400',
      iconPath: 'M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z',
    },
    warning: {
      bg: 'bg-warning-50 dark:bg-warning-900/30 border-warning-200 dark:border-warning-700/50',
    ...
    info: {
      bg: 'bg-info-50 dark:bg-info-900/30 border-info-200 dark:border-info-700/50',
  ...
  const { bg, icon, iconPath } = styles[toast.type];
  ...
  return (
    <div className={`pointer-events-auto flex items-start gap-3 p-3 rounded-lg border shadow-lg ${bg} animate-slide-in-right`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {toast.message && (
          <p className="text-xs text-surface-600 dark:text-surface-400 mt-0.5">{toast.message}</p>
        )}
      </div>
      <button
        onClick={() => onDismiss(toast.id)}
        className="flex-shrink-0 p-0.5 rounded text-surface-700 hover:text-surface-900 dark:text-surface-400 dark:hover:text-surface-300 transition-colors"
      >
