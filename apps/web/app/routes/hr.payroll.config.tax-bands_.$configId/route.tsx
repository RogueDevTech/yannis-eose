...
import { PayrollTaxBandViewPage } from '~/features/hr/PayrollTaxBandViewPage';
...
export const meta: MetaFunction = () => [{ title: 'PAYE tax band — Yannis EOSE' }];
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (!configId) throw redirect('/hr/payroll/config/roles?tab=tax');
  ...
  const pageData = (async () => {
    ...
    if (!config) throw redirect('/hr/payroll/config/roles?tab=tax');
...
export default function PayrollTaxBandDetailRoute() {
  const { pageData } = useLoaderData<typeof loader>();
  return (
    <CachedAwait resolve={pageData} fallback={<PayrollConfigLoadingShell />} loaderShell={{}} deferredKey="pageData">
      {(data) => <PayrollTaxBandViewPage config={data.config} canWrite={data.canWrite} />}
    </CachedAwait>
  );
}
