...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const productsData = productsRes.ok
    ? ((productsRes.data as { result?: { data?: { products: { id: string; name: string }[] } } })?.result?.data ?? null)
    : null;

  const locationsData = locationsRes.ok
    ? ((locationsRes.data as { result?: { data?: { locations: { id: string; name: string; providerName?: string | null }[] } } })
        ?.result?.data ?? null)
    : null;
