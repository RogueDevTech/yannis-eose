-- Add resolve_receipt_url to orders for required receipt upload when 3PL resolves order (date/cost/discount).

-- ── orders ────────────────────────────────────────────────────
ALTER TABLE orders
  ADD COLUMN IF NOT EXISTS resolve_receipt_url text;
...
BEGIN
  INSERT INTO orders_history (
    ...
    delivery_notes, delivery_proof_url, delivery_discount_amount, resolve_receipt_url, parent_order_id,
    ...
    NEW.delivery_notes, NEW.delivery_proof_url, (NEW.delivery_discount_amount)::numeric, NEW.resolve_receipt_url, NEW.parent_order_id,
