...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  // Deferred: invoice summary (un-awaited promise with .catch fallback)
  ...
  // Deferred: approvals (un-awaited promise with .catch fallback)
  ...
  const approvals = approvalsParsed.then((d) => d.requests);
  const totalApprovals = approvalsParsed.then((d) => d.total);
  const pendingApprovals = approvalsParsed.then((d) =>
    d.requests.filter((r) => r.status === 'PENDING' || r.status === 'QUERIED').length,
  );

  // Deferred: budgets (un-awaited promise with .catch fallback)
  const budgets = budgetsPromise.then((res) => {
    if (!res.ok) return [];
    return (res.data as { result?: { data?: Budget[] } })?.result?.data ?? [];
  }).catch(() => [] as Budget[]);

  // v3_singleFetch: return plain object — no json() wrapper needed
  return {
    ...
    filters: { startDate: startDate ?? '', endDate: endDate ?? '', invoiceStatus: invoiceStatus ?? '', approvalStatus: approvalStatus ?? '' },
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === 'createInvoice') {
    ...
    if (!res.ok) {
      ...
      return json({ error: errorData?.error?.message ?? 'Failed to create invoice' }, { status: res.status });
    }
  ...
  if (intent === 'updateInvoiceStatus') {
    ...
    if (!res.ok) {
      const errorData = res.data as { error?: { message?: string } };
      return json({ error: errorData?.error?.message ?? 'Failed to update invoice status' }, { status: res.status });
    }
    return json({ success: true });
  }
  ...
  if (intent === 'processApproval') {
    ...
    if (!res.ok) {
      const errorData = res.data as { error?: { message?: string } };
      return json({ error: errorData?.error?.message ?? 'Failed to process approval' }, { status: res.status });
    }
    return json({ success: true });
  }
  ...
  if (intent === 'flagOverdueInvoices') {
    ...
    if (!res.ok) {
      return json({ error: 'Failed to flag overdue invoices' }, { status: res.status });
    }
